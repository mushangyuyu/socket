...
public class AESUtil{
    ...
    private static final String keyBytes = "abcdefgabcdefg12"; 
    	//随机生成一个16位的aes密钥,这个类中没有被用到，但是是一个不错的方法
    public static String generateRandomAesKey(){
		String base = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
		String aeskey = "";
		Random random = new Random();
		for(int i = 0 ; i < 16 ; i ++){
			aeskey += base.charAt(random.nextInt(base.length()));
		}
		return aeskey;
	}
    /**
    *加密
    */
    public static String encode(String content,String str){
            //加密之后的字节数组,转成16进制的字符串形式输出
        return parseByte2HexStr(encrypt(content, str));
    }
    ...
    */
    public static String decode(String content,String str) throws UnsupportedEncodingException{
            //解密之前,先将输入的字符串按照16进制转成二进制的字节数组,作为待解密的内容输入
        byte[] b = decrypt(parseHexStr2Byte(content), str);
        return new String(b);
    }
